...
class FaceDetectors:
    ...
    def __init__(self):
        ...
        self.model_head_pose = pickle.load(open('D:\PyCharm\pythonProject\Face_Rec\model.pkl', 'rb'))
...
### ################# START CODE MAIN ############################
    ...
    def face_detector(self):
        ...
        screen_height_px = monitor.height
        width_cm = monitor.width_mm / 10
        height_cm = monitor.height_mm / 10
        print(f"Màn hình: {monitor.name}")
        print(f"  Kích thước vật lý: {width_cm:.2f} cm x {height_cm:.2f} cm")
        print(f"  Độ phân giải: {screen_width_px} x {screen_height_px} pixels\n")

        # Tính 9/10 chiều rộng và chiều cao --> tiêu cự focal = 1500 (1280->1920)
        frame_width = int(screen_width_px * (9 / 10))
        frame_height = int(screen_height_px * (9 / 10))

        # # Tính 9/10 chiều rộng và chiều cao --> tiêu cự focal = 1500 (1280->1920)
        # frame_width = int(screen_width * (0.45))
        # frame_height = int(screen_height * (0.45))

        # frame_width = screen_width_px
        # frame_height = screen_height_px

        # print(f"screen_width: {screen_width_px}, screen_height{screen_height_px}")
...
############### ######### START CODE HEAD POSE ###############
                ...
                pitch_pred, yaw_pred, roll_pred = 15.0, 15.0, 15.0
...
################### ######### START tính khoảng cách mặt ###############
                    ...
                    if frame_width <= 400:
                        ...
                        focal_length = 1200  # Phù hợp cho ảnh ~640x480
                        ...
                        focal_length = 1500  # Phù hợp cho HD 720p
                        print("Kích thước HD → focal ~ 1500px")

                    else:
                        focal_length = 1700  # Phù hợp cho FullHD trở lên
...
################### ######### START mắt đang nhìn đâu trên màn hình point xanh ###############
                    ...
                    # print("Gaze Point:", gaze_point_new)
                    cv2.circle(frame, gaze_point_new, 20, (0, 255, 0), -1)
...
################### ######### START mắt đang nhìn đâu trên màn hình point đỏ ###############
                    ...
                    #
                    # lr_redpoint = 1
                    # dx = -bbox_width * np.sin(pitch) * np.cos(yaw)*lr_redpoint  # Sử dụng công thức mới cho dx
                    # dy = -bbox_width * np.sin(yaw)*lr_redpoint  # Sử dụng công thức mới cho dy


                    do_lech_face = f"degrees: {str(round(np.abs(np.degrees(yaw_pred)), 2))}"
                    cv2.putText(frame, do_lech_face, (25, 250), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)

                    # if np.abs(pitch) + np.abs(yaw) > 0.7:
                    if np.abs(yaw_pred) > 0.4:

                        def compute_gaze_offset_dynamic(pitch, yaw, bbox_width, base_lr=0.7):
                            # Tính góc tổng hợp để xác định mức độ liếc
                            angle_magnitude = np.abs(pitch) + np.abs(yaw)
                            print(f"angle_magnitude: {angle_magnitude}")

                            # Điều chỉnh lr_redpoint theo độ liếc — liếc nhiều thì tăng hệ số
                            # Giới hạn min/max để tránh nhảy quá xa
                            dynamic_lr = base_lr + 0.82 * angle_magnitude # 0.7 khá là ổn
                            # dynamic_lr = np.clip(dynamic_lr, 0.15, 0.5)  # Điều chỉnh ngưỡng nếu cần
                            dynamic_lr = np.clip(dynamic_lr, 0.15, 0.45)  # Điều chỉnh ngưỡng nếu cần
                        ...
                        dx, dy = compute_gaze_offset_dynamic(pitch, yaw, bbox_width, 0.5)
                    else:
                        lr_redpoint = 0.75
                    ...
                    # image_height, image_width = new_bbox_height, new_bbox_width
...
################### ######### START vẽ phạm vi nhận hướng của mắt ###############
                    ...
                    bottom_y_min = new_y_min + int(image_height * 0.70)
...
################### ######### START Cảnh báo cheating #################
                    ...
                    # elif quadrant == "top" and quadrant == "bottom":
                    #     cheating = f"Cheating"
                    #     cv2.putText(frame, cheating, (25, 170), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 255), 3)
